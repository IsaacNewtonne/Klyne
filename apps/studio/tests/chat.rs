...
fn chat_repairs_invalid_ollama_plan() {
    for invalid in [complete("Hi!"), json!({"tasks":[]}), json!({"question":""})] {
        let s = Server::new();
        let (endpoint, fixture) = model(vec![invalid, plan(), complete("Hello!"), complete("Hello!")]);
        ...
        assert!(requests[0]["messages"][0]["content"].as_str().unwrap().contains("Your current role is PLANNER"));
        assert!(requests[1]["messages"][1]["content"].as_str().unwrap().contains("validation_error"));
    }
}
...
fn contract_rejects_model_success_without_the_requested_artifact() {
    let s=Server::new();
    let (endpoint,fixture)=model(vec![plan(),complete("Written"),complete("Everything passed"),complete("Everything passed")]);
    let mut body=request(&endpoint);body["message"]=json!("write file required.txt :: expected");
    let created=s.api("/api/chats",Some(body));let id=created["id"].as_str().unwrap();let chat=s.wait(id);
    assert_eq!(chat["status"],"Blocked","{chat}");
    assert_eq!(chat["result"]["outcome"],"unmet");
    assert_eq!(chat["contract"]["criteria"][0]["FileContents"]["expected"],"expected");
    assert_eq!(chat["result"]["checks"][0]["passed"],false);
    let calls=fixture.join().unwrap();
    assert!(calls.last().unwrap()["messages"][1]["content"].as_str().unwrap().contains("Task acceptance failed"));
    let mut resume=request(&endpoint);resume["id"]=json!(id);resume["resume"]=json!(true);
    resume["contract"]=json!({"goal":"different","criteria":[]});
    assert!(s.api("/api/chats",Some(resume))["error"].as_str().unwrap().contains("Cannot replace"));
    assert_eq!(s.api(&format!("/api/chats/{id}"),None)["contract"],chat["contract"]);
}

#[test]
fn contract_verifies_actual_file_and_survives_loading() {
    let s=Server::new();
    let (endpoint,fixture)=model(vec![plan(),json!({"decision":"act","action":{"tool":"write_file","path":"proof.txt","contents":"checked"}}),complete("Written"),complete("Here is proof.txt")]);
    let mut body=request(&endpoint);body["message"]=json!("Create the requested artifact");
    body["contract"]=json!({"goal":"Create the requested artifact","criteria":[{"FileContents":{"path":"proof.txt","expected":"checked"}}]});
    let created=s.api("/api/chats",Some(body));let id=created["id"].as_str().unwrap();let chat=s.wait(id);
    assert_eq!(chat["status"],"Completed","{chat}");assert_eq!(chat["result"]["outcome"],"verified");
    assert_eq!(chat["result"]["checks"][0]["passed"],true);
    assert_eq!(s.api(&format!("/api/chats/{id}"),None)["result"],chat["result"]);
    fixture.join().unwrap();
}
...
fn restart_reconciles_saved_document_from_file_without_desktop_input() {
    ...
    let created=s.api("/api/chats",Some(body));let id=created["id"].as_str().unwrap();
    let paused=s.wait(id);assert_eq!(paused["status"],"Blocked");
    s.child.kill().unwrap();s.child.wait().unwrap();
    std::fs::write(std::path::Path::new(paused["workspace"].as_str().unwrap()).join("note.txt"),"saved contents").unwrap();
    let db=rusqlite::Connection::open(s.root.path().join("conversations").join(id).join("chat.sqlite3")).unwrap();
    let payload:String=db.query_row("SELECT payload FROM chat WHERE id=1",[],|r|r.get(0)).unwrap();
    let mut payload:Value=serde_json::from_str(&payload).unwrap();
    payload["status"]=json!("Working");payload["tasks"][0]["status"]=json!("Working");
    ...
    db.execute("UPDATE chat SET payload=?1 WHERE id=1",[payload.to_string()]).unwrap();drop(db);
    s.child=Command::new(env!("CARGO_BIN_EXE_klyne-studio")).args(["--port",s.host.split(':').nth(1).unwrap(),"--root"]).arg(s.root.path()).stdout(Stdio::null()).spawn().unwrap();
    let deadline=Instant::now()+Duration::from_secs(15);
    while TcpStream::connect(&s.host).is_err(){assert!(Instant::now()<deadline);std::thread::sleep(Duration::from_millis(20));}
    let done=s.wait(id);assert_eq!(done["status"],"Completed","{done}");assert!(done["pending"].is_null());
    assert!(done["evidence"].as_array().unwrap().iter().any(|e|e["action"]=="document_save_reconcile"));
    assert!(!done["evidence"].as_array().unwrap().iter().any(|e|e["action"]=="desktop_key"||e["action"]=="desktop_observe"));
...
fn api_connection_failure_switches_once_to_desktop_without_replaying() {
    ...
    let (endpoint,fixture)=model(vec![plan(),json!({"decision":"act","action":{"tool":"app_connect","name":"offline","base_url":origin}}),call.clone(),call,complete("Fallback inspected"),json!({"decision":"fail","reason":"Fixture app has no desktop window"})]);
    let mut body=request(&endpoint);body["access"]=json!({"apps":true,"desktop":true});
    let created=s.api("/api/chats",Some(body));let chat=s.wait(created["id"].as_str().unwrap());
    assert_eq!(chat["status"],"Blocked","{chat}");
    assert_eq!(chat["execution"]["desktop_fallbacks"],json!(["api:offline"]));
    assert!(chat["pending"].is_null());
    let events=chat["evidence"].as_array().unwrap();
    assert_eq!(events.iter().filter(|e|e["action"]=="app_call").count(),1);
...
fn missing_input_resumes_the_same_step_without_replanning() {
    let s=Server::new();
    let (endpoint,fixture)=model(vec![plan(),json!({"decision":"needs_input","question":"Which title should I use?"}),complete("Title accepted"),complete("Finished")]);
    ...
    answer["id"]=json!(id);answer["message"]=json!("Use Project notes");
...
fn dependency_graph_executes_ready_steps_and_preserves_repair_history() {
    ...
    let calls=fixture.join().unwrap();
    assert!(calls[1]["messages"][1]["content"].as_str().unwrap().contains("Produce input"));
    let messages=chat["messages"].as_array().unwrap();
    let first=messages.iter().position(|m|m["text"]=="Input ready").unwrap();
    let second=messages.iter().position(|m|m["text"]=="Result used").unwrap();
    assert!(first<second);
}
...
fn recovery_uses_an_independent_provider_and_preserves_the_primary_choice() {
    ...
    fs::write(s.root.path().join("recovery/config.json"), json!({"enabled":true,"fallback":{"kind":"ollama","endpoint":fallback,"model":"backup"}}).to_string()).unwrap();
    let chat = s.api("/api/chats", Some(request(&primary)));
    let chat = s.wait(chat["id"].as_str().unwrap());
    assert_eq!(chat["status"], "Completed", "{chat}");
    assert_eq!(chat["provider"]["endpoint"], primary);
    assert!(chat["messages"].as_array().unwrap().iter().any(|m| m["agent"] == "Recovery"));
...
fn recovery_records_an_exhausted_model_failure_without_replaying_tools() {
    ...
    assert_eq!(chat["status"], "Blocked", "{chat}");
    assert!(chat["evidence"].as_array().unwrap().is_empty());
    let incidents = fs::read_dir(s.root.path().join("recovery/incidents")).unwrap().collect::<Vec<_>>();
    assert_eq!(incidents.len(), 1);
    let incident: Value = serde_json::from_slice(&fs::read(incidents[0].as_ref().unwrap().path()).unwrap()).unwrap();
    assert_eq!(incident["failure"]["kind"], "model_request");
    assert_eq!(incident["failure"]["pending"], Value::Null);
    assert_eq!(incident["failure"]["messages_count"].as_u64().unwrap() + 1, chat["messages"].as_array().unwrap().len() as u64);
...
fn recovery_screenshot_link_opens_the_failed_conversation() {
    ...
    browser.navigate(&format!("http://{}/#chat={id}", s.host)).unwrap();
    browser_wait(&mut browser, "snapshot?.status==='Completed'");
    assert_eq!(browser.eval("selected").unwrap(), id);
    assert_eq!(browser.eval("recoveryLink.textContent").unwrap(), "Recovery");
    fixture.join().unwrap();
}
...
fn chat_creates_loads_and_reuses_versioned_skills_and_tools() {
    ...
    let first = s.wait(first["id"].as_str().unwrap());
    ...
    let second = s.wait(second["id"].as_str().unwrap());
    assert_eq!(second["status"], "Completed", "{second}");
    let requests = fixture.join().unwrap();
    assert!(
        requests[7]["messages"][1]["content"]
            .as_str()
            .unwrap()
...
fn production_view_tracks_real_model_workers_evidence_and_result() {
    ...
    assert_eq!(
        b.eval("document.querySelectorAll('#prod-edges path').length>=3")
            .unwrap(),
        true
    );
    fs::write(
    ...
    fs::write(artifacts.join("production-to-chat-morph.png"),b.screenshot().unwrap()).unwrap();
    browser_wait(&mut b,"document.querySelector('#production').dataset.view==='conversation' && __morphPending===0");
    ...
    fs::write(artifacts.join("chat-to-production-morph.png"),b.screenshot().unwrap()).unwrap();
    browser_wait(&mut b,"document.querySelector('#production').dataset.view==='production' && __morphPending===0");
    assert_eq!(b.eval("(!__nativeMorph || __morphCount===2) && document.querySelectorAll('.prod-morph-vessel').length===0").unwrap(),true);
    b.eval("for(let i=0;i<4;i++)document.querySelector('#prod-view-toggle').click()").unwrap();
    browser_wait(&mut b,"__morphPending===0 && document.querySelector('#production').dataset.view==='production'");
    ...
    browser_wait(&mut b,"document.querySelector('#production').dataset.view==='conversation'");
...
fn chat_browser_conversation_settings_controls_and_mobile() {
    ...
    assert_eq!(b.eval("document.querySelector('#apps #api-connect-form')===null").unwrap(),true);
    b.eval("document.querySelector('#apps').close()").unwrap();
    b.click("#settings-button").unwrap();
    b.eval("document.querySelector('#api-connect-form').closest('details').open=true").unwrap();
