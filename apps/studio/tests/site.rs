...
fn public_demo_runs_without_backend_and_responds_to_controls() {
    ...
    let path=root.join("site/index.html").to_string_lossy().replace('\\',"/").trim_start_matches("//?/").to_owned();
    browser.navigate(&format!("file:///{path}")).unwrap();
    let deadline=Instant::now()+Duration::from_secs(15);
    loop {
        if browser.eval("typeof snapshot!=='undefined' && snapshot?.status==='Completed'").unwrap_or_default()==serde_json::json!(true){break;}
        assert!(Instant::now()<deadline,"Demo did not complete");std::thread::sleep(Duration::from_millis(100));
    }
    ...
    assert_eq!(browser.eval("document.querySelector('#prod-details').getAttribute('aria-pressed')==='true'").unwrap(),true);
    browser.click("#prod-details").unwrap();
