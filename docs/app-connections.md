...
The initial executable catalog supports Google Chrome and Microsoft Edge through Microsoft's `@playwright/mcp` **0.0.80**. `mcp_setup` installs that exact package under the runtime's `conversations/mcp-packages`, with npm lifecycle scripts disabled and a 90-second deadline. Installation is an explicit worker tool call with Apps access — discovery, reads, and calls never install anything, and reviewers cannot install. Node/npm must already be installed. The npm lockfile records dependency integrity. Existing packages with a different version are rejected rather than silently updated.

Verification performs MCP initialization, protocol negotiation, paginated tool discovery, and a real browser tab-list operation. Each later call must name a discovered tool. Each conversation gets an isolated headless browser profile (`mcp-profile-{server}-{conversation}`); tabs, cookies, and storage never cross conversations, and a maximum of four live sessions evicts least-recently-used ones (child reaped, profile removed). This does not attach to existing signed-in tabs or grant full control of browser settings. Authentication may still need user involvement. Reviewers can inspect tool definitions but cannot invoke arbitrary MCP operations.
...
MCP calls preserve their per-conversation session. The host rejects unsupported client requests, caps message size and pagination, imposes a 45-second request deadline, and uses a Windows job to own the server's descendant processes. An interrupted or disconnected tool call is recorded as uncertain and is not replayed through desktop fallback; a dead session's profile is removed. Setup failures before dispatch can fall back safely.
