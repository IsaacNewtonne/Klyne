...
A false-success regression exposed a model claiming Zalo delivery with all external access disabled and zero tool calls. The host now rejects recognized delivery completion claims because no production delivery-receipt adapter is available. It also rejects common action-completion phrases without successful evidence from the current task. The original request and evidence start index persist across resume. These conservative English-language checks are an interim defense, not a general semantic proof of arbitrary claims; they may block requests that discuss delivery and do not replace operation-specific verification. The offending local run was corrected while retaining its original history.
