...
An existing matching file establishes a postcondition, not historical causation. Advisory locks and lexical/reparse-point checks retain the limitations documented in README. Reconciliation reads now reserve persisted tool-call credit before invocation, including failed reads, and enforce the filesystem tool's 1 MiB ceiling. Reservation failure prevents the read; budget exhaustion leaves pending state intact. Active legacy checkpoints lacking accounting refuse continuation. A terminal run still returns its historical outcome without revalidation. IDs are unique only within a run/database; event consumers must retain database/run identity when aggregating records.
